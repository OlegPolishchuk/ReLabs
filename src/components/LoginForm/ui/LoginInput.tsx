...
interface Props extends ComponentPropsWithRef<'input'>{
  ...
  register: (name: 'email', options?: RegisterOptions<LoginInputs, 'email'>) => UseFormRegisterReturn<'email'>}
...
export const LoginInput = ({errors, register, disabled}: Props) => {
  ...
  return (
    <Input.Wrapper className={cls.label} label="Электронаая почта" required
                   error={loginError}>
      <Input
        className={cls.input}
